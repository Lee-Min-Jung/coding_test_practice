...
### 회고
<ul>
<li>bfs는 최단 거리를 보장하기 때문에 bfs돌면서 최단 거리 저장하려고 Math.min을 해 주지 않아도 된다 이론상으로는 이해가 되는데 직관적으로는 이해가 가지 않는 상태</li>
<li>bfs의 while에서 큐에서 하나 뽑은 후 방문 배열 처리를 하면 시간 초과 에러가 나지만 동서남북 살피는 for문 내부에서 큐에 다음 좌표 넣기 전에 방문 배열 처리 하면 시간 초과 에러가 나지 않는다. 후자의 경우 이미 방문 배열 처리를 해서 덜 돌아서 그런다고 하는데 이해가 될 듯 말 듯....</li>
</ul>

### 성능 요약

메모리: 52.4 MB, 시간: 5.15 ms
